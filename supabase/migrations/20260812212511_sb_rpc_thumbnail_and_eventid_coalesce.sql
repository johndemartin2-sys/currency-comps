-- Vault the current (guarded) version before touching it
insert into rollback.artifacts (name, kind, content)
select 'fn_ingest_stacks_bowers_lot_guards_20260812', 'ddl', pg_get_functiondef(p.oid)
from pg_proc p join pg_namespace n on n.oid=p.pronamespace
where n.nspname='public' and p.proname='ingest_stacks_bowers_lot'
on conflict (name) do nothing;

-- v2 changes: (1) p_thumbnail_url param (DEFAULT NULL - old callers unaffected);
-- (2) auction_event_id/name/thumbnail COALESCE on update so API-mode nulls
-- can never erase values already on the row.
