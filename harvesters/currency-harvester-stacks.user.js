...
//     thumbnail writes landed (mechanics were otherwise perfect: 0 err, 2
//     unsold correctly labeled, 0 mismatches). Fixed; the 192 burned
//     worklist rows had raw.enriched_at cleared server-side so they recycle.
//   * WORKLIST ORDER (server-side get_sb_enrich_worklist v2): rows NEEDING A
//     PRICE now come first (colonials first within that), then thumb-only
//     rows. The first run surfaced recent thumb-only colonials before the
//     old unpriced ones - prices are the point of this pass.
// v9.7.0 (2026-08-20): ENRICH MODE - repair pass for first-run harvest gaps.
//   32,638 SB rows have no price (v8 scraped AngularJS app shells that never
//   render realized prices in static HTML) and 65,083 DOM-era rows have no
//   thumbnail. The public per-lot JSON endpoint
//     GET https://auctions.stacksbowers.com/ajax/lot/{LOT_CODE}
//   (LOT_CODE = source_lot_id, from /lots/view/{CODE}) returns
//   status ('sold'/'expired'), sold_price (HAMMER), buyers_premium_amount
//   (exact premium; NULL on pre-~2012 auctions whose sold_price is already
//   all-in) and images[]. Verified against 12 known rows across 2005-2026:
//   sold_price + coalesce(buyers_premium_amount,0) matched stored
//   prices-realized to the penny in every era.
//   FLOW: the panel's Enrich button pulls worklist batches from the proxy
//   (get_sb_enrich_worklist: price-or-thumb-missing, not yet attempted,
//   colonials first), fetches /ajax/lot/ per code at ~1.5s + jitter, and
//   posts enrich_stacks_bowers_lot (narrow writes: fills NULL price, labels
//   unsold in raw.lot_status, backfills thumbnail; flags >1% price
//   mismatches for review instead of overwriting). Every attempted row gets
//   raw.enriched_at, so the worklist never recycles. Stop/Resume supported;
//   401 stops the run. ~77K lots at defaults = a few evenings.
...
//     before:  POST /rest/v1/rpc/ingest_stacks_bowers_lot   apikey: <anon>
//     after:   POST /functions/v1/ingest-proxy/ingest_stacks_bowers_lot
...
//   Boot guard: refuses to harvest until the key placeholder is replaced.
//   SHARED CORE untouched, still byte-identical with Heritage.
// v9.5.2 (2026-08-18): STATE MAP GAP - District of Columbia added. The state
//   extractor validated 2-letter codes against STATE_ABBRS, which was derived
//   from a 50-state map, so "Washington, DC-" titles always landed with a
//   NULL state. DC (and Puerto Rico, which Heritage's map already carries)
//   are now recognised by both the abbreviation and full-name paths. The
//   SHARED CORE block is untouched and stays byte-identical with Heritage.
...
(function () {
  ...
  // v9.7.0 ENRICH mode
  const WORKLIST_ENDPOINT = SUPABASE_URL + '/functions/v1/ingest-proxy/get_sb_enrich_worklist';
  const ENRICH_ENDPOINT   = SUPABASE_URL + '/functions/v1/ingest-proxy/enrich_stacks_bowers_lot';
  const AJAX_LOT_BASE     = 'https://auctions.stacksbowers.com/ajax/lot/';
  const ENRICH_BATCH      = 100;     // worklist rows per proxy call
  const ENRICH_DELAY_MS   = 1500;    // pause between /ajax/lot fetches (+/-25% jitter)
  /* ================================================================== *
  ...
  const STATE_FULLNAMES = {
    ...
    // v9.5.2: DC and PR were missing, so "Washington, DC-" lots lost their state
   ...
   * v9.6.0: through the ingest-proxy edge function. The private key travels
   * in x-harvest-key; no Supabase API key appears in this script at all.
   * The proxy passes the RPC's status and body straight through, so the
   * ins:/upd:, reject: and breaker handling below is unchanged.
   * ------------------------------------------------------------------ */
  ...
  function proxyPost(url, payload) {
    return new Promise((resolve) => {
      GM_xmlhttpRequest({
        ...
        onload: (resp) => {
          let body = null;
          try { body = JSON.parse(resp.responseText); } catch(e){ body = resp.responseText; }
  ...
  function fetchAjaxLot(code) {
    return new Promise((resolve) => {
      GM_xmlhttpRequest({
        method: 'GET', url: AJAX_LOT_BASE + encodeURIComponent(code),
        headers: { 'Accept': 'application/json' },
        onload: (resp) => {
          if (resp.status < 200 || resp.status >= 300) return resolve({ err: 'HTTP ' + resp.status });
          try {
            const j = JSON.parse(resp.responseText);
            resolve({ d: (j && j.response) || j });
          } catch(e){ resolve({ err: 'bad json' }); }
        },
        onerror:   () => resolve({ err: 'network' }),
        ontimeout: () => resolve({ err: 'timeout' })
      });
    });
  }
  function extractImageUrl(d) {
    ...
    if (i && typeof i === 'object') {
      for (const k of ['thumbnail_url','detail_url','image_url','url','large','medium','thumbnail','thumb','src']) {
        if (typeof i[k] === 'string' && /^https?:/.test(i[k])) return i[k];
      }
    }
    return null;
  }
  function enFRESH(){ return { fetched:0, priced:0, unsold:0, thumb:0, mismatch:0, miss:0, err:0 }; }
  async function enrichLoop(){
    if (!st.en) st.en = enFRESH();
    while (st.running && st.mode === 'enrich') {
      const wl = await proxyPost(WORKLIST_ENDPOINT, { p_limit: ENRICH_BATCH });
      if (wl.status === 401) { logErr('KEY REJECTED (401) - check HARVEST_KEY.'); return finish('ENRICH stopped - key rejected'); }
      if (wl.status < 200 || wl.status >= 300 || !Array.isArray(wl.body)) {
        logErr('WORKLIST ERR ' + wl.status + ' ' + String(JSON.stringify(wl.body)).slice(0,100));
        return finish('ENRICH stopped - worklist error');
      }
      if (!wl.body.length) return finish('ENRICH complete - worklist empty');
      for (const item of wl.body) {
        if (!st.running || st.mode !== 'enrich') return;
        const code = item.source_lot_id;
        const a = await fetchAjaxLot(code);
        st.en.fetched++;
        if (a.err) {
          st.en.err++; logErr('AJAX ' + a.err + ' :: ' + code);
        } else {
          const d = a.d || {};
          const r = await proxyPost(ENRICH_ENDPOINT, {
            p_source_lot_id: code,
            p_status: d.status || null,
            p_sold_price: (d.sold_price != null && isFinite(parseFloat(d.sold_price))) ? parseFloat(d.sold_price) : null,
            p_buyers_premium: (d.buyers_premium_amount != null && isFinite(parseFloat(d.buyers_premium_amount))) ? parseFloat(d.buyers_premium_amount) : null,
            p_thumbnail_url: extractImageUrl(d)
          });
          if (r.status === 401) { logErr('KEY REJECTED (401).'); return finish('ENRICH stopped - key rejected'); }
          const out = String(r.body || '');
          if (r.status >= 200 && r.status < 300) {
            if (/^"?priced/.test(out)) st.en.priced++;
            else if (/^"?unsold/.test(out)) st.en.unsold++;
            else if (/^"?thumb/.test(out)) st.en.thumb++;
            else if (/^"?mismatch/.test(out)) { st.en.mismatch++; logErr('PRICE MISMATCH flagged :: ' + code); }
            else if (/^"?miss/.test(out)) st.en.miss++;
          } else {
            st.en.err++; logErr('ENRICH ERR ' + r.status + ' ' + out.slice(0,90) + ' :: ' + code);
          }
        }
        st.msg = 'enrich: ' + st.en.fetched + ' fetched - priced ' + st.en.priced +
          ' - unsold ' + st.en.unsold + ' - thumb ' + st.en.thumb +
          (st.en.mismatch ? ' - MISMATCH ' + st.en.mismatch : '') +
          ' - miss ' + st.en.miss + ' - err ' + st.en.err;
        save(); paint();
        await sleep(Math.round(ENRICH_DELAY_MS * (0.75 + Math.random() * 0.5)));
      }
    }
  }
  function startEnrich(){
    if (keyMissing()){ st.msg = 'HARVEST KEY NOT SET - edit the CONFIG block.'; return paint(); }
    st = Object.assign(FRESH(), { mode: 'enrich', running: true, en: enFRESH(),
      msg: 'enrich started - pulling worklist (colonials first) ...' });
    save(); paint(); enrichLoop();
  }
  async function upsertWithRetry(payload) {
    ...
    if (!r.ok && r.status === 401) return r;                                   // bad/revoked key: never retry
    ...
    async function worker(){
      while (true){
        ...
        if (res.ok){
          ...
          if (res.status === 401){
            r.breaker = true; logErr('KEY REJECTED (401) - check HARVEST_KEY. Run stopped.'); return;
          }
  ...
  function buildPanel(){
    ...
    p.innerHTML =
      ...
      '<div class="row"><b>DOM</b> <button id="sbh9-run" title="Fallback: harvest every page by clicking the pager">Run All Pages</button> ' +
      '<button id="sbh9-enrich" title="Repair pass: fills missing prices (+ labels unsold) and thumbnails from the /ajax/lot endpoint, colonials first. Self-driving worklist; runs from any archive page.">Enrich</button></div>' +
    ...
    el('sbh9-enrich').onclick = startEnrich;
    ...
    el('sbh9-resume').onclick = function(){ st.running = true; st.msg = 'resumed'; save(); paint();
      if (st.mode === 'enrich') enrichLoop(); else runLoop(); };
  ...
  function boot(){
    ...
    buildPanel();
    const tf = seriesSelfTest('sbh9.6') + classifySelfTest('sbh9.6');
    ...
    if (st.running){ st.msg = 'resuming after reload ...'; save();
      if (st.mode === 'enrich'){ enrichLoop(); return; }
