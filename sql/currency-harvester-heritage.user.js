HERITAGE HARVESTER v9.5.0 — paste this whole file over your existing Heritage
script in Tampermonkey, including the // ==UserScript== header.

You will know it loaded when the panel header reads "Currency Harvester v9.5.0"
and the console shows:  [cch9.5] series self-test ok (17 cases)
If it shows SELFTEST FAIL, the panel will say "Do not sweep" — stop and tell Claude.

This pairs with Stack's v9.5.1: both carry the SHARED CORE block byte-for-byte.
If you ever edit the parser regexes, edit them in BOTH files.
...
// @version      9.5.0
// @description  Coin-harvester engine (sweep + top up + self-pagination) with v8 currency extraction -> ingest_heritage_lot
...
const SUPABASE_ANON_KEY = 'sb_publishable_9DiOeDQje6yiSEITD7WUrw_S2Q-2xyS';
const RPC_URL = 'https://' + SUPABASE_REF + '.supabase.co/rest/v1/rpc/ingest_heritage_lot';
...
function parseRow(li) {
  ...
  // equals its Friedberg base after this release -- the residual ~506 case.
...
function findRows() {
  ...
  return rows;
    ...
    // 4xx rejects and breaker trips are NEVER retried.
    ...
    async function tick(){
      ...
      tally(r); save(); paint();
      if (r.breaker)
        return finish('CIRCUIT BREAKER tripped - the DB is refusing unknown-Fr inserts at systemic rate. Investigate stub flood (analytics_fr_stubs) before resuming.');
    ...
    function boot(){
      buildPanel(); paint();
      const tf = seriesSelfTest('cch9.5');
      if (tf){ st.msg = 'SERIES SELF-TEST FAILED (' + tf + ') - see console. Do not sweep.'; paint(); }
      if (SUPABASE_ANON_KEY.indexOf('PASTE_') === 0){ st.msg = 'anon key not set'; paint(); }
