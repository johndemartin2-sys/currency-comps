...
//
// Pay-first flow: a brand-new customer may not have a Supabase profile at the
// moment payment completes, so resolveUserId also falls back to matching the
// profile by EMAIL (the email Stripe collected at checkout). Once the account
// is created with that same email, the entitlement reconciles.
//
...
// Resolve the Supabase user id from explicit ids, the Stripe customer, or email.
async function resolveUserId({ userId, customerId, email }) {
...
async function handle({ customerId, userId, email, products, stripeStatus, subscriptionId }) {
  const resolvedUserId = await resolveUserId({ userId, customerId, email });
